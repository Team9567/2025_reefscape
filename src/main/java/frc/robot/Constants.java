// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot;

import static edu.wpi.first.units.Units.Inches;
import static edu.wpi.first.units.Units.Meters;

/**
 * The Constants class provides a convenient place for teams to hold robot-wide
 * numerical or boolean
 * constants. This class should not be used for any other purpose. All constants
 * should be declared
 * globally (i.e. public static). Do not put anything functional in this class.
 *
 * <p>
 * It is advised to statically import this class (or one of its inner classes)
 * wherever the
 * constants are needed, to reduce verbosity.
 */
public final class Constants {
  public enum CanDevices {
    ChassisLeftRear(1),
    ChassisLeftFront(2),
    ChassisRightRear(3),
    ChassisRightFront(4),
    ElevatorLiftMotor(5),
    ElevatorLiftMotorFollower(6),
    IntakeMotor(7);

    public final int id;

    private CanDevices(int id) {
      this.id = id;
    }
  }

  public static class OperatorConstants {
    public static final int kDriverControllerPort = 0;
  }

  public static class ControllerConstants {
    public static final int kControllerControllerPort = 1;
  }

  public static class ButtonConstants {
    public static final int kButtonX = 3;
    public static final int kButtonY = 4;
    public static final int kButtonB = 2;
    public static final int kButtonA = 1;
    public static final int kButtonLB = 5;
    public static final int kButtonRB = 6;
    public static final int kAxisLT = 2;
    public static final int kAxisRT = 3;
    public static final int kButtonLeftStick = 9;
    public static final int kButtonRightStick = 10;
    public static final int kButtonBack = 7;
    public static final int kButtonStart = 8;
  }

  public static class ChassisConstants {
    public static final int kDriverControllerPort = 0;
    public static final double kWheelDiameterMeters = Meters.convertFrom(6.17, Inches);
    public static final double kGearRatio = 8.46;
    public static final double kPositionConversionFactor = (kWheelDiameterMeters * Math.PI) / kGearRatio;
    public static final double kMotorRampTime = 0.1;
    public static final double kDriveClamp = 0.5;
    public static final double kTurnClamp = 0.5;
    public static final double kLowGearSpeed = 0.50;
  }

  public static class ElevatorConstants {
    public static final boolean kMotorInverted = false;
    public static final boolean kMotorFollowerInverted = false;
    public static final double kGearRatio = 1.0;
    public static final double kWheelDiameterMeters = Meters.convertFrom(4.0, Inches);
    // aiming for units of meters for position and meters per minute for velocity
    public static final double kPositionConversionFactor = (kWheelDiameterMeters * Math.PI) / kGearRatio;
    public static final double kMotorRampTime = 0.1;
    public static final double kPV = 0.20;
    public static final double kIV = 0;
    public static final double kDV = 0;
    public static final double kS = 0.20;
    public static final double kG = 0;
    public static final double kV = 0;
    public static final double kA = 0;
  }

  public static class IntakeConstants {
    public static final boolean kMotorInverted = false;
    public static final int kMotorCurrentLimit = 80;
    public static final double kMotorVoltageCompensation = 10.0;
    public static final double kGearRatio = 1.0;
    public static final double kWheelDiameterMeters = Meters.convertFrom(4.0, Inches);
    public static final double kPositionConversionFactor = (kWheelDiameterMeters * Math.PI) / kGearRatio;
    public static final double kMotorRampTime = 0.1;
    public static final double kPV = 0.20;
    public static final double kIV = 0;
    public static final double kDV = 0;
    public static final double kS = 0.18;
    public static final double kV = 0.14;
  }
}
